...
def get_daily_data(ticker_symbol: str, start_date: str, end_date: str):
    """Get daily close prices for a ticker symbol with specified date range."""
...
def calculate_start_date(period: str, final_year: int = 2025) -> str:
    """Calculate start date based on period and final year."""
    if period not in AVAILABLE_PERIODS:
        raise ValueError(f"Period must be one of {list(AVAILABLE_PERIODS.keys())}")

    years_back = AVAILABLE_PERIODS[period]
    end_date = datetime(final_year, 12, 31)
    start_year = end_date.year - years_back
    start_date = end_date.replace(year=start_year)
    return start_date.strftime("%Y-%m-%d")


def get_end_date(final_year: int = 2025) -> str:
    """Get end date string from final year."""
